...
class ExpansionPlanningModel:
    ...
    def create_model(self):
        ...
        m.dispatchPeriodLength = Param(within=PositiveReals, default=1, units=u.hr)
...
def commitment_period_rule(b, commitment_period):
    ...
    if False:
       ...
       m.loads = {
             m.md.data["elements"]["load"][load_n]["bus"]: m.md.data["elements"]["load"][load_n]["p_load"]["values"][commitment_period - 1]
             for load_n in m.md.data["elements"]["load"]
         }
       for key, val in m.loads.items():
            # print(f"{key=}")
            # print(f"{val=}")
            m.loads[key] *= m.config['alpha_scaler']
...
def model_data_references(m):
    ...
    m.storageInvestmentCost = {
        bat: m.md.data["elements"]["storage"][bat]["investment_cost_kwh"]*m.md.data["elements"]["storage"][bat]["energy_capacity"]*1000
        for bat in m.storage
